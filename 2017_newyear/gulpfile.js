...
gulp.task('cleanTempFiles', function() {
    return gulp.src(releasePath('_tempfile', ASSET_PATH), {
            read: false
        })
        .pipe(clean())
});
...
gulp.task('concatJsAndTpl', function() {
    var frameVars = ['$scope', '$router', '_$dom', '$Signal', '_$scopeMgr', '$htmlLoader', '$libsLoader', '_$scopeLoading', '_$ctor'];
    return gulp.src([releasePath('js/*.js', ASSET_PATH), releasePath('_tempfile/*.js', ASSET_PATH)])
        .pipe(concat('index.min.js'))
        .pipe(
            uglify({
                mangle: {
                    except: frameVars.concat(ignoreVars)
                }
            })
        )
...
function releasePath(url_, path_) {
    if (typeof url_ === 'string') {
        ...
        var arr = [];
        for (var i in url_) {
            var u = RELEASE_PATH + '/' + (path_ ? path_ + '/' : '') + url_[i];
            arr.push(u);
        }
        return arr;
    }
...
gulp.task('Release', gulpSequence('clean', ["copy", "build", 'html2js', "copyPackCss"], 'fixcss', "concatJsAndTpl", "cleanTempFiles", 'base64css'));
